...
const calculateAverage = (average: number[]): number => average.reduce((previousValue, currentValue) => previousValue + currentValue) / average.length; 
...
const summarizeClassRoom = (studentList: Student[]) : StudentSummary[] => {
  ...
  studentList.forEach(student => {
      result.push({
        ...
        averageCalifications: Number(calculateAverage(student.califications).toFixed(2))
      });
  })
